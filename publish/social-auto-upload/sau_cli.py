...
async def upload_bilibili_video(request: BilibiliVideoUploadRequest) -> BilibiliVideoUploadResult:
    ...
    if request.tags:
        base_arguments.extend(["--tag", ",".join(request.tags)])
    if isinstance(request.publish_date, datetime):
        base_arguments.extend(["--dtime", str(int(request.publish_date.timestamp()))])
    if request.thumbnail_file:
        base_arguments.extend(["--cover", str(request.thumbnail_file)])

    submit_modes = [request.submit_api, "b-cut-android", "web", "app"]
    submit_modes = list(dict.fromkeys(mode for mode in submit_modes if mode))
    last_error = ""
    tried_modes: list[str] = []
    for submit_mode in submit_modes:
        tried_modes.append(submit_mode)
        result = run_biliup_command([*base_arguments, "--submit", submit_mode])
        output = (result.stdout or "").strip()
        error = (result.stderr or "").strip()
        if result.returncode == 0:
            return BilibiliVideoUploadResult(
                account_file=account_file,
                submit_api=submit_mode,
                output=output or error,
            )
        last_error = error or output or "Bilibili upload failed"
        if "21150" not in last_error and "投稿入口升级中" not in last_error:
            break

    if len(tried_modes) > 1:
        last_error = f"{last_error}\nTried Bilibili submit APIs: {', '.join(tried_modes)}"
    raise RuntimeError(last_error or "Bilibili upload failed")
    return BilibiliVideoUploadResult(account_file=account_file, submit_api=request.submit_api)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    upload_video_parser.add_argument("--preview", action="store_true", help="Preview mode: fill form but don't submit, wait for manual publish")
    ...
    kuaishou_upload_video_parser.add_argument("--preview", action="store_true", help="Preview mode: fill form but don't submit, wait for manual publish")
    ...
    xiaohongshu_upload_video_parser.add_argument("--draft", action="store_true", help="Save to draft instead of publishing")
    xiaohongshu_upload_video_parser.add_argument("--thumbnail", type=existing_file_path, help="Optional thumbnail path")
    xiaohongshu_upload_video_parser.add_argument("--preview", action="store_true", help="Preview mode: fill form but don't submit, wait for manual publish")
    ...
    xiaohongshu_upload_note_parser.add_argument("--draft", action="store_true", help="Save to draft instead of publishing")
    ...
    bilibili_upload_video_parser.add_argument("--thumbnail", type=existing_file_path, help="Optional thumbnail path")
    bilibili_upload_video_parser.add_argument(
        "--submit",
        default="app",
        choices=["app", "web", "b-cut-android"],
        help="Bilibili submit API, defaults to app and falls back on known entry-upgrade errors",
    )
    return parser
...
async def dispatch(args: argparse.Namespace) -> int:
    ...
    if args.platform == "xiaohongshu":
        ...
        publish_strategy = (
            XIAOHONGSHU_PUBLISH_STRATEGY_DRAFT
            if args.draft
            else (XIAOHONGSHU_PUBLISH_STRATEGY_SCHEDULED if args.schedule else XIAOHONGSHU_PUBLISH_STRATEGY_IMMEDIATE)
        )
    ...
    if args.platform == "bilibili":
        ...
        if args.action == "upload-video":
            request = BilibiliVideoUploadRequest(
                ...
                publish_date=args.schedule or 0,
                thumbnail_file=args.thumbnail,
                submit_api=args.submit,
            )
            result = await upload_bilibili_video(request)
            print(f"Bilibili video upload submitted: {request.video_file}")
            print(f"Bilibili submit API: {result.submit_api}")
            if result.output:
                print(f"Bilibili output: {result.output}")
            return 0
