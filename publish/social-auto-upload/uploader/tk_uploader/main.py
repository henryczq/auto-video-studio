    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        try:
            await page.wait_for_selector(
                'iframe[data-tt="Upload_index_iframe"], div.upload-container, div.upload-stage-container, input[type="file"], button[data-e2e="select_video_button"], button:has-text("Select video"), button:has-text("选择视频"), button:has-text("Upload")',
                timeout=10000,
            )
        ...
        file_input = self.locator_base.locator('input[type="file"]').first
        if await file_input.count():
            await file_input.set_input_files(self.file_path)
        else:
            upload_button = self.locator_base.locator(
                'button[data-e2e="select_video_button"]:visible, button:has-text("Select video"):visible, button:has-text("选择视频"):visible, button:has-text("Upload"):visible, button[aria-label="Select file"]:visible, button[aria-label="选择视频"]:visible')
