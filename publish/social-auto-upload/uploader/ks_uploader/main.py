...
class KSVideo(KSBaseUploader):
    ...
    async def set_thumbnail(self, page: Page):
        ...
        # 快手封面确认后还会异步写回主表单；过早点击发布可能沿用默认视频帧。
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        try:
            ...
            if self.preview:
                kuaishou_logger.info(_msg("👀", "预览模式：视频和内容已填充完毕，请检查后手动点击「发布」按钮"))
                kuaishou_logger.info(_msg("💡", "发布完成后关闭浏览器即可，脚本会自动保存 cookie"))
                # 等待用户操作或超时
                try:
                ...
                try:
                    ...
                    kuaishou_logger.success(_msg("🥳", "预览模式结束，cookie 已更新"))
                except Exception:
                    pass
                await context.close()
                await browser.close()
