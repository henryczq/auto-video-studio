...
async def baijiahao_cookie_gen(account_file):
    async with async_playwright() as playwright:
        ...
        for _ in range(120):
            ...
            if "login" not in page.url and not await page.get_by_text('注册/登录百家号').count():
        ...
        await context.storage_state(path=account_file)
        await browser.close()
        baijiahao_logger.warning("登录等待超时，已保存当前 cookie，请检查是否登录成功")
...
class BaiJiaHaoVideo(object):
    ...
    async def open_video_editor(self, page):
        target_url = "https://baijiahao.baidu.com/builder/rc/edit?type=videoV2"
        last_error = None
        for _ in range(2):
            try:
                ...
                last_error = exc
        raise RuntimeError(f"打开百家号发布页失败，最后错误: {last_error}")

    async def trigger_upload_entry(self, page):
        candidates = [
            page.get_by_role("button", name="上传视频"),
            page.get_by_text("上传视频", exact=False),
            page.get_by_text("点击上传", exact=False),
            page.get_by_text("选择视频", exact=False),
            page.locator("div[class*='upload']").first,
            page.locator("div[class*='video']").first,
        ]
                ...
                try:
                    ...
                    for index in range(count):
                        ...
                        try:
                            ...
                            baijiahao_logger.info(f"已命中文件上传输入框: {selector}[{index}]")
                            return
                        except Exception as exc:
                            last_error = exc
                except Exception as exc:
                    last_error = exc
            if attempt == 0:
                await self.trigger_upload_entry(page)

        screenshot_msg = "截图未生成"
        screenshot_path = "logs/baijiahao_upload_timeout.png"
        try:
            await page.screenshot(path=screenshot_path, full_page=True)
            screenshot_msg = f"截图: {screenshot_path}"
        except Exception as screenshot_exc:
            screenshot_msg = f"截图失败: {screenshot_exc}"
        page_title = ""
        try:
            ...
            page_title = ""
        raise RuntimeError(
            f"未找到百家号视频上传 input，url={page.url}，title={page_title}，{screenshot_msg}，最后错误: {last_error}"
        )
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        page = await context.new_page()
        # 访问指定的 URL
        await self.open_video_editor(page)
        ...
        # 判断视频封面图是否生成成功；页面结构变化时，发布按钮可用也视为可继续
        await self.wait_cover_ready(page)
    ...
    async def direct_publish(self, page):
        try:
            await self.dismiss_location_popups(page)
            publish_button = page.get_by_test_id("publish-btn")
            if await publish_button.count() == 0:
                publish_button = page.get_by_role("button", name="发布", exact=True)
            if await publish_button.count() == 0:
                publish_button = page.locator("button").filter(has_text="发布").last
            await self.click_publish_button(page, publish_button)
        except Exception as e:
            baijiahao_logger.error(f"直接发布视频失败: {e}")
            raise  # 重新抛出异常，让重试装饰器捕获

    async def dismiss_location_popups(self, page):
        selectors = [
    ...
    async def click_publish_button(self, page, button):
        last_error = None
        try:
        ...
        screenshot_msg = "截图未生成"
        screenshot_path = "logs/baijiahao_publish_timeout.png"
        try:
            await page.screenshot(path=screenshot_path, full_page=True)
            screenshot_msg = f"截图: {screenshot_path}"
        except Exception as screenshot_exc:
        ...
        raise RuntimeError(
            f"百家号发布按钮点击失败，url={page.url}，{screenshot_msg}，最后错误: {last_error}"
        )
    ...
    async def wait_publish_success(self, page):
        ...
        for _ in range(60):
            ...
            if "builder/rc/clue" in current_url or "builder/rc/manage" in current_url:
                return
            ...
            await page.wait_for_timeout(1000)
        screenshot_msg = "截图未生成"
        screenshot_path = "logs/baijiahao_publish_state_timeout.png"
        try:
            await page.screenshot(path=screenshot_path, full_page=True)
            screenshot_msg = f"截图: {screenshot_path}"
        except Exception as screenshot_exc:
            screenshot_msg = f"截图失败: {screenshot_exc}"
        raise RuntimeError(f"发布后未检测到成功状态，当前 url={page.url}，{screenshot_msg}")

    async def wait_cover_ready(self, page):
        cover_selectors = [
            "div.cheetah-spin-container img",
            "img[alt*='封面']",
            "div[class*='cover'] img",
        ...
        for _ in range(20):
            ...
            try:
                if await publish_button.count():
                    is_disabled = await publish_button.first.is_disabled()
                    if not is_disabled:
                        baijiahao_logger.info("发布按钮已可用，继续后续流程...")
                        return
            except Exception:
                pass
            baijiahao_logger.info("等待封面生成...")
            await asyncio.sleep(3)
        baijiahao_logger.warning("封面检测超时，继续尝试发布")
    ...
    async def add_title_tags(self, page):
        ...
        title = self.title[:30]
        candidates = [
            page.get_by_placeholder('添加标题获得更多推荐'),
            page.get_by_placeholder('请输入标题'),
            page.get_by_placeholder('请填写标题'),
            page.get_by_placeholder('标题'),
            page.locator('input[maxlength="30"]:visible').first,
            page.locator('input[placeholder*="标题"]:visible').first,
            page.locator('textarea[placeholder*="标题"]:visible').first,
            page.locator('div#formMain input:visible').first,
            page.locator('div#formMain textarea:visible').first,
        ...
        for candidate in candidates:
            try:
                if await candidate.count() == 0:
                    continue
                await candidate.fill(title, timeout=5000)
                return
            except Exception as exc:
                last_error = exc
                try:
        ...
        screenshot_msg = "截图未生成"
        try:
            await page.screenshot(path="logs/baijiahao_title_timeout.png", full_page=True)
            screenshot_msg = "截图: logs/baijiahao_title_timeout.png"
        except Exception as screenshot_exc:
            screenshot_msg = f"截图失败: {screenshot_exc}"
        raise RuntimeError(
            f"未找到百家号标题输入框，url={page.url}，{screenshot_msg}，最后错误: {last_error}"
        )
