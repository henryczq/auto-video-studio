...
XIAOHONGSHU_PUBLISH_STRATEGY_DRAFT = "draft"
...
class XiaoHongShuBaseUploader(BaseVideoUploader):
    ...
    async def validate_base_args(self):
        ...
        if self.publish_strategy not in {
            ...
            XIAOHONGSHU_PUBLISH_STRATEGY_DRAFT,
        }:
    ...
    async def save_draft(self, page: Page, content_type: str) -> None:
        draft_buttons = [
            page.locator('button:has-text("保存到草稿")').first,
            page.locator('button:has-text("保存为草稿")').first,
            page.locator('button:has-text("保存草稿")').first,
            page.locator('button:has-text("存草稿")').first,
            page.locator('button:has-text("草稿箱")').first,
        ]
        last_error = None
        for _ in range(8):
            for button in draft_buttons:
                try:
                    ...
                    xiaohongshu_logger.success(_msg("🥳", f"{content_type}已保存到草稿箱"))
                    return
                except Exception as exc:
                    last_error = exc
            xiaohongshu_logger.info(_msg("🏃", f"小人正在尝试保存{content_type}草稿"))
            if self.debug:
                await page.screenshot(full_page=True)
            await asyncio.sleep(0.5)
        raise RuntimeError(f"未找到小红书草稿按钮，无法保存{content_type}草稿: {last_error}")
...
class XiaoHongShuVideo(XiaoHongShuBaseUploader):
    ...
    async def set_thumbnail(self, page: Page, thumbnail_path: str):
        ...
        cover_preview = cover_plugin_title.locator(
            "xpath=ancestor::div[contains(@class, 'cover-plugin-preview')]"
        )
        clicked = await click_first_visible(
            [
                ("旧版封面上传入口", cover_preview.locator("div.cover > div.default:visible")),
                ("封面预览卡片", page.locator("div[class*='cover-plugin-preview']").first),
                ("设置封面文字", page.get_by_text("设置封面", exact=True)),
                ("上传封面文字", page.get_by_text("上传封面", exact=True)),
                ("更换封面文字", page.get_by_text("更换封面", exact=True)),
            ],
            timeout=8000,
        )
        if not clicked:
            raise RuntimeError("未找到小红书封面设置入口")
        xiaohongshu_logger.info(_msg("🧭", f"已点击封面入口: {clicked}"))

        modal = page.locator(
            "div.d-modal.cover-modal, div[role='dialog']:has-text('封面'), div[class*='modal']:has-text('封面')"
        ).first
        try:
            await modal.wait_for(state="visible", timeout=15000)
        except Exception:
            xiaohongshu_logger.warning(_msg("😵", "未看到封面弹窗，小人尝试直接查找页面里的图片上传控件"))
            modal = page

        tab_clicked = await click_first_visible(
            [
                ("上传封面页签", modal.get_by_text("上传封面", exact=True)),
                ("本地上传页签", modal.get_by_text("本地上传", exact=True)),
                ("从电脑上传页签", modal.get_by_text("从电脑上传", exact=True)),
            ],
            timeout=3000,
        )
        if tab_clicked:
            xiaohongshu_logger.info(_msg("🧭", f"已切换到: {tab_clicked}"))
        ...
        xiaohongshu_logger.info(_msg("🖼️", "封面图片已经提交给页面"))
        await page.wait_for_timeout(2000)

        confirm_clicked = await click_first_visible(
            [
                ("确定按钮", modal.locator("button").filter(has_text="确定")),
                ("完成按钮", modal.locator("button").filter(has_text="完成")),
                ("保存按钮", modal.locator("button").filter(has_text="保存")),
                ("应用按钮", modal.locator("button").filter(has_text="应用")),
            ],
            timeout=5000,
        )
        if not confirm_clicked:
            raise RuntimeError("封面已上传，但未找到确认按钮")
        xiaohongshu_logger.info(_msg("🧭", f"已点击封面确认按钮: {confirm_clicked}"))

        try:
            if modal is not page:
                await modal.wait_for(state="hidden", timeout=20000)
        except Exception:
            xiaohongshu_logger.warning(_msg("😵", "封面弹窗没有自动关闭，小人继续后续发布流程"))
    ...
    async def upload_video_content(self, page: Page) -> None:
        ...
        while True:
            try:
                ...
                desc_input = page.locator('p[data-placeholder*="输入正文描述"]').first
                publish_button = page.locator('button:has-text("发布")').first
                if (
                    await title_input.count()
                    and await desc_input.count()
                    and await publish_button.count()
                    and await title_input.is_visible()
                    and await desc_input.is_visible()
                    and await publish_button.is_visible()
                ):
                    xiaohongshu_logger.success(_msg("🥳", "视频发布编辑区已就绪，开始填写内容"))
                    break
        ...
        try:
            ...
            xiaohongshu_logger.warning(_msg("😵", f"封面设置失败，继续使用平台默认封面: {exc}"))
        ...
        if self.publish_strategy == XIAOHONGSHU_PUBLISH_STRATEGY_DRAFT:
            await self.save_draft(page, "视频")
            return

        # 预览模式：停在发布确认页面，等待用户手动点击发布
        if self.preview:
            xiaohongshu_logger.info(_msg("👀", "预览模式：视频和内容已填充完毕，请检查后手动点击「发布」按钮"))
            xiaohongshu_logger.info(_msg("💡", "发布完成后关闭浏览器即可，脚本会自动保存 cookie"))
            # 等待用户操作或超时
            try:
            ...
            try:
                ...
                xiaohongshu_logger.success(_msg("🥳", "预览模式结束，cookie 已更新"))
            except Exception:
                pass
            return
